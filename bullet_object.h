...
struct bullet_object_{
    ...
    bullet_object_(SDL_Renderer* renderer, SDL_Texture* bullet_texture_mob, const float& x_mob , const float& y_mob, const int& mob_size, const float& vector_xmob_to_main, const float& vector_ymob_to_main){
        bullet_texture = bullet_texture_mob;
        SDL_QueryTexture(bullet_texture, NULL, NULL, NULL, &bullet_size);

        x_bullet_on_map = x_mob + mob_size/2 + vector_xmob_to_main*(mob_size/2);
        y_bullet_on_map = y_mob + mob_size/2 + vector_ymob_to_main*(mob_size/2);
    ...
    bool daim_on_main(const SDL_FRect& main_hitbox, int& main_hp);
};
